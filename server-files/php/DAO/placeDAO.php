...
class PlaceDAO
{
   ...
   public function insertPlace(Place $place) {
       ...
       $COLUMNS =  '(' . self::COL_ID . ',' . self::COL_AMENITY . ','
       . self::COL_DESCRIPTION . ',' . self::COL_LATITUDE . ',' . self::COL_LOCNAME. ','
       . self::COL_LONGITUDE . ',' . self::COL_NAME . ',' . self::COL_SHORTNAME . ')';
      ...
      $SQL->bind_param('issdsdss', $id,$amenity,$description,
           $latitude, $locName,$longitude,$name,$shortName);
      $SQL->execute();

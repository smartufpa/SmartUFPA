...
<section ng-app="moderationApp" ng-controller="moderationController" ng-submit="submitPlaces()">
	<h2>Locais inseridos</h2>
	<hr>
	<!-- CARDS	 -->
	<?php if(count($placesToModerate) == 0){ ?> 	<!-- Se não há lugares para moderar	 -->
	
	<h3>Nenhum local para moderação!</h3>
	
	<?php }else {?> <!-- Se há lugares para moderar	carrega os cards e o formulário -->
	...
	<section>
		<input type="checkbox" style="float: left" ng-model="place[<?php echo $ID ?>].checked"  >
		<div class="card row">
				<input hidden ng-init="place[<?php echo $ID?>].id= '<?php echo $ID;?>'">
				<input hidden ng-init="place[<?php echo $ID?>].checked=false">
				...
				<input hidden ng-init="addToPlaces(place[<?php echo $ID ?>])">
		</div>
		<br>
		<div class="row">
...
<script src="js/moderation-app.js"></script>
